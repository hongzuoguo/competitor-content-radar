...
describe('China-time scheduler', () => {
  it('schedules the daily run at the next 09:00 China time', () => {
  ...
  it('requests only one catch-up after a missed daily run', () => {
    ...
    expect(shouldRunCatchUp(new Date('2026-07-09T01:00:00.000Z'), now, false)).toBe(true)
    expect(shouldRunCatchUp(new Date('2026-07-09T01:00:00.000Z'), now, true)).toBe(false)
    expect(shouldRunCatchUp(new Date('2026-07-11T01:00:00.000Z'), now, false)).toBe(false)
  })
})
