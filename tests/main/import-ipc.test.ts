...
function dependencies(): IpcDependencies {
  return {
    ...
    saveSettings: vi.fn(), startImport: vi.fn(), retryImport: vi.fn(), deleteFailedWork: vi.fn(),
...
describe('import IPC', () => {
  ...
  it('passes a sanitized import request and validates retry ids', async () => {
    ...
    expect(deps.startImport).toHaveBeenCalledWith({
      source: { type: 'local', path: 'clip.mp4' },
      creatorId: null,
      ownership: 'mine'
    })
  ...
  it('extracts the first Douyin work URL from complete share text before calling the service', async () => {
    const deps = dependencies()
    vi.mocked(deps.startImport).mockResolvedValue({ accepted: true, workId: 'work-1' })
    registerIpcHandlers(deps)

    await handlers.get(IPC_CHANNELS.importStart)?.({}, {
      source: {
        type: 'douyin_url',
        url: '4.16 复制打开抖音，看看【示例】 https://www.douyin.com/user/MS4wLjABAAAA 作品：https://www.douyin.com/video/7531234567890123456 更多内容'
      },
      creatorId: 'creator-1'
    })

    expect(deps.startImport).toHaveBeenCalledWith({
      source: { type: 'douyin_url', url: 'https://www.douyin.com/video/7531234567890123456' },
      creatorId: 'creator-1',
      ownership: 'mine'
    })
  })
  ...
  it.each([
    ['a creator profile', 'https://www.douyin.com/user/MS4wLjABAAAA'],
    ['text without a link', '复制打开抖音，看看这个作品']
  ])('rejects %s at the import boundary without calling the service', async (_case, url) => {
    const deps = dependencies()
    registerIpcHandlers(deps)
  ...
  it('continues to accept a bare Douyin work URL', async () => {
    const deps = dependencies()
    vi.mocked(deps.startImport).mockResolvedValue({ accepted: true, workId: 'work-1' })
    registerIpcHandlers(deps)

    await handlers.get(IPC_CHANNELS.importStart)?.({}, {
      source: { type: 'douyin_url', url: ' https://v.douyin.com/AbCd123/ ' }, creatorId: null
    })

    expect(deps.startImport).toHaveBeenCalledWith({
      source: { type: 'douyin_url', url: 'https://v.douyin.com/AbCd123/' },
      creatorId: null,
      ownership: 'mine'
    })
  })

  it.each(['mine', 'competitor'] as const)('passes through the supported %s ownership', async (ownership) => {
    const deps = dependencies()
    vi.mocked(deps.startImport).mockResolvedValue({ accepted: true, workId: 'work-1' })
    registerIpcHandlers(deps)
  ...
  it.each([null, '', 'Mine', 'other', 1, {}, []])('rejects invalid ownership %#', async (ownership) => {
    const deps = dependencies()
    registerIpcHandlers(deps)
    ...
    await expect(handlers.get(IPC_CHANNELS.importStart)?.({}, {
      ...
      ok: false,
      error: { code: 'INVALID_IMPORT_REQUEST', retryable: false }
    })
    expect(deps.startImport).not.toHaveBeenCalled()
  })
