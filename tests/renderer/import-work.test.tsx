...
describe('work import dialog', () => {
  ...
  it('imports a local video with an optional creator', async () => {
    ...
    await waitFor(() => expect(desktopApi.startImport).toHaveBeenCalledWith({
      source: { type: 'local', path: 'C:\\video\\样片.mp4' }, creatorId: 'creator-1', ownership: 'mine'
    }))
    expect(await screen.findByText('任务已启动，请到作品分析查看进度')).toBeInTheDocument()
  })

  it('defaults to my work and can import a competitor work explicitly', async () => {
    render(<WorksPage />)
    fireEvent.click(screen.getByRole('button', { name: '导入作品' }))
    expect(await screen.findByRole('radio', { name: '我的作品' })).toBeChecked()
    fireEvent.click(screen.getByRole('radio', { name: '对标作品' }))
  ...
  it('keeps creator selection when switching source tabs', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
  ...
  it('shows the Douyin share field and rejects a creator profile URL', async () => {
    render(<WorksPage />)
    fireEvent.click(screen.getByRole('button', { name: '导入作品' }))
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    const shareField = screen.getByLabelText('抖音分享内容或作品链接')
    expect(shareField.tagName).toBe('TEXTAREA')
    expect(screen.getByText('可粘贴完整分享口令或单条公开作品链接，不支持纯博主主页。')).toBeInTheDocument()
    fireEvent.change(shareField, { target: { value: 'https://www.douyin.com/user/test' } })
    fireEvent.click(screen.getByRole('button', { name: '开始分析' }))
    expect(await screen.findByText('请输入抖音作品分享内容，不支持纯博主主页。')).toBeInTheDocument()
    expect(desktopApi.startImport).not.toHaveBeenCalled()
  })

  it.each([
    'https://www.douyin.com/user/test?from_tab_name=main',
    'https://www.douyin.com/user/test#works'
  ])('shows the creator profile error for a profile URL with tracking data: %s', async (profileUrl) => {
    render(<WorksPage />)
    fireEvent.click(screen.getByRole('button', { name: '导入作品' }))
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    fireEvent.change(screen.getByLabelText('抖音分享内容或作品链接'), { target: { value: profileUrl } })
    fireEvent.click(screen.getByRole('button', { name: '开始分析' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('请输入抖音作品分享内容，不支持纯博主主页。')
    expect(desktopApi.startImport).not.toHaveBeenCalled()
  })
  ...
  it.each([
    ...
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    fireEvent.change(screen.getByLabelText('抖音分享内容或作品链接'), { target: { value: url } })
    fireEvent.click(screen.getByRole('button', { name: '开始分析' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('没有在分享内容中识别到抖音作品链接。')
    expect(desktopApi.startImport).not.toHaveBeenCalled()
  })
  ...
  it('accepts a work opened from a creator modal', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    fireEvent.change(screen.getByLabelText('抖音分享内容或作品链接'), { target: { value: modalUrl } })
    fireEvent.click(screen.getByRole('button', { name: '开始分析' }))
    await waitFor(() => expect(desktopApi.startImport).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('accepts a direct video URL and a valid short URL', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    const input = screen.getByLabelText('抖音分享内容或作品链接')
  ...
  it('extracts the first work URL from a complete Douyin share message', async () => {
    render(<WorksPage />)
    fireEvent.click(screen.getByRole('button', { name: '导入作品' }))
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    fireEvent.change(screen.getByLabelText('抖音分享内容或作品链接'), {
      target: { value: '5.24 复制打开抖音，看看【测试作品】 https://v.douyin.com/XBND3GOR4Fo/ 备用链接 https://www.douyin.com/video/7658288075461725474' }
    })
    fireEvent.click(screen.getByRole('button', { name: '开始分析' }))
    await waitFor(() => expect(desktopApi.startImport).toHaveBeenCalledWith(expect.objectContaining({
      source: { type: 'douyin_url', url: 'https://v.douyin.com/XBND3GOR4Fo/' }
    })))
  })

  it('shows a specific error when share text has no Douyin work URL', async () => {
    render(<WorksPage />)
    fireEvent.click(screen.getByRole('button', { name: '导入作品' }))
    fireEvent.click(screen.getByRole('button', { name: '抖音分享' }))
    await waitForCreators()
    fireEvent.change(screen.getByLabelText('抖音分享内容或作品链接'), {
      target: { value: '这是一段没有作品链接的分享文本。' }
    })
    fireEvent.click(screen.getByRole('button', { name: '开始分析' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('没有在分享内容中识别到抖音作品链接。')
    expect(desktopApi.startImport).not.toHaveBeenCalled()
  ...
  it('switches source tabs with the keyboard', async () => {
    ...
    expect(screen.getByRole('button', { name: '抖音分享' })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByLabelText('抖音分享内容或作品链接')).toBeInTheDocument()
  })
})
