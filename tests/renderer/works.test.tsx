...
const completed: WorkListItem = {
  ...
  sourceType: 'douyin_monitor', ownership: 'competitor', publishedAt: '2026-07-13T08:20:00.000Z', status: 'completed', stage: 'completed',
  errorCode: null, errorMessage: null, retryable: false, likes: 18_642, relativePerformanceMultiplier: 2.4,
  canAnalyzeManually: false, reasons: ['absolute_high_likes', 'relative_performance']
}

const processing: WorkListItem = {
  id: 'work-running', creatorId: null, creatorName: '未分类作品', title: '本地样片', sourceType: 'local_file',
  ownership: 'mine', publishedAt: '2026-07-13T09:00:00.000Z', status: 'running', stage: 'transcribed', errorCode: null,
  errorMessage: null, retryable: false, likes: 0, relativePerformanceMultiplier: null, canAnalyzeManually: false, reasons: []
}
...
describe('work analysis library', () => {
  ...
  it('shows only my account in the source rail when filtering my works', async () => {
    const mine = {
      ...completed,
      id: 'work-mine', creatorId: 'creator-mine', creatorName: '我的主页', ownership: 'mine' as const
    }
    desktopApi = createDesktopApi([completed, mine])
    desktopApi.listCreators = vi.fn().mockResolvedValue([
      { id: 'creator-complete', name: '对标来源', profileUrl: 'https://example/competitor', enabled: true, works: 1, lastRun: '刚刚', status: 'ready', ownership: 'competitor' },
      { id: 'creator-mine', name: '我的主页', profileUrl: 'https://example/mine', enabled: true, works: 1, lastRun: '刚刚', status: 'ready', ownership: 'mine' }
    ])
    Object.defineProperty(window, 'desktopApi', { configurable: true, value: desktopApi })
    render(<WorksPage />)
    ...
    expect(await screen.findByRole('button', { name: /^我的主页/ })).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /^对标来源/ })).not.toBeInTheDocument()
  })
  ...
  it('filters viral works without hiding operational states from all', async () => {
    desktopApi.listWorks = vi.fn().mockResolvedValue([completed, processing, failed])
    render(<WorksPage />)
  ...
  it('separates my works from competitor works', async () => {
    desktopApi.listWorks = vi.fn().mockResolvedValue([completed, processing])
    render(<WorksPage />)
    await screen.findByText(completed.title)
    ...
    fireEvent.click(screen.getByRole('button', { name: '对标作品' }))
    expect(await screen.findByText(completed.title)).toBeInTheDocument()
    expect(screen.queryByText('本地样片')).not.toBeInTheDocument()
  })
  ...
  it('ignores historical duplicates on initial loads and unrelated events', async () => {
    ...
    await waitFor(() => expect(desktopApi.listWorks).toHaveBeenCalledTimes(2))
    await waitFor(() => expect(screen.getByRole('button', { name: '爆款' })).toHaveAttribute('aria-pressed', 'true'))
  ...
  it.each([
    ...
    ['FFMPEG_BINARY_MISSING', 'analyzed', '安装包缺少音频处理组件，请重新安装最新版 HitMuse。'],
