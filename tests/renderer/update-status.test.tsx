...
describe('automatic update status', () => {
  ...
  it('retains accessible labels for compact update feedback', () => {
    const { rerender } = render(<UpdateStatus initialState={{ status: 'downloading', percent: 42 }} />)
    expect(screen.getByLabelText('正在下载更新 42%')).toBeInTheDocument()
    rerender(<UpdateStatus initialState={{ status: 'error', message: 'failed' }} />)
