...
describe('public privacy gate', () => {
  it('initializes the private tool cache from an exact SID descriptor without localized icacls output', async () => {
    ...
    expect(source).toContain('[Security.AccessControl.RawSecurityDescriptor]::new($sddl)')
    expect(source).toContain('[Security.AccessControl.ControlFlags]::DiscretionaryAclProtected')
    expect(source).not.toContain('[regex]::Match($sddl')
