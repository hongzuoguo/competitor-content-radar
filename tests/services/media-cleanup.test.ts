...
describe('local media handling', () => {
  ...
  it('never follows an ordinary file symlink', ({ skip }) => {
    ...
    try {
      ...
      // Windows without Developer Mode / admin rights reports EPERM, EACCES
      // or UNKNOWN when creating file symlinks. The cleanup behavior under
      // test cannot run in that environment, so skip instead of failing.
      if (error instanceof Error && 'code' in error && (error.code === 'EPERM' || error.code === 'EACCES' || error.code === 'UNKNOWN')) skip()
      throw error
    }
