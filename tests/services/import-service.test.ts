...
describe('ImportService', () => {
  ...
  it('marks changed import data and keeps completed work when the settled sync fails', async () => {
    const onLocalDataChanged = vi.fn()
    const flushAfterTask = vi.fn().mockRejectedValue(Object.assign(new Error('Feishu unavailable'), { code: 'FEISHU_SYNC_FAILED' }))
    const deps = dependencies({ onLocalDataChanged, afterSettled: flushAfterTask })
    const service = new ImportService(deps)

    const started = await service.start({
      source: { type: 'local', path: 'source.mp4' },
      creatorId: null
    })
    await service.shutdown()

    expect(repositories.works.get(started.workId)?.ownership).toBe('mine')
    expect(repositories.jobs.get(started.workId)).toMatchObject({ stage: 'completed', status: 'completed' })
    expect(deps.processor.analyze).toHaveBeenCalledTimes(1)
    expect(onLocalDataChanged).toHaveBeenCalledTimes(3)
    expect(flushAfterTask).toHaveBeenCalledOnce()
    expect((deps as Record<string, unknown>).afterAnalyzed).toBeUndefined()
  })
  ...
  it('uses the selected analysis engine for a manual import', async () => {
    const deps = dependencies()
    const analyzeSelected = vi.fn(async () => ({
      result: { referenceValueScore: 90 }, provider: 'local-agent', model: 'codex',
      promptVersion: 'agent-v1', tokenUsage: null
    }))
    Object.assign(deps.processor, { analyzeSelected })
    deps.getSettings = vi.fn(() => ({ runEngine: 'local-agent' }))
    const service = new ImportService(deps)

    const started = await service.start({
      source: { type: 'local', path: 'source.mp4' }, creatorId: null
    })
    await vi.waitFor(() => expect(repositories.jobs.get(started.workId)?.status).toBe('completed'))

    expect(analyzeSelected).toHaveBeenCalledWith(
      started.workId,
      'transcript',
      { runEngine: 'local-agent' }
    )
    expect(deps.processor.analyze).not.toHaveBeenCalled()
    expect(repositories.analyses.get(started.workId)?.provider).toBe('local-agent')
  })
  ...
  it('keeps analysis and resumes from synced when final completion persistence fails', async () => {
    const deps = dependencies()
    ...
    if (!started.accepted) throw new Error('expected accepted import')
    await vi.waitFor(() => expect(repositories.jobs.get(started.workId)?.status).toBe('failed'))
    expect(repositories.jobs.get(started.workId)?.stage).toBe('synced')
    expect(repositories.analyses.get(started.workId)?.transcript).toBe('transcript')
  ...
  it('deletes a failed task only after its managed work directory is removed', async () => {
    ...
    try {
      ...
      const onLocalDataChanged = vi.fn()
      const service = new ImportService(dependencies({ mediaRoot, onLocalDataChanged }))
      ...
      expect(onLocalDataChanged).toHaveBeenCalledOnce()
    } finally {
      rmSync(mediaRoot, { recursive: true, force: true })
    }
  })

  it('rolls back failed-work deletion when persisting the local change marker fails', async () => {
    repositories.works.upsert({
      id: 'failed-marker', creatorId: null, platformWorkId: null, sourceType: 'local_file',
      sourceKey: 'sha256:failed-marker', mediaPath: null, title: 'Failed marker',
      publishedAt: '2026-07-12T00:00:00.000Z', originalUrl: null, downloadUrl: null,
      metrics: { likes: 0, comments: 0, shares: 0, collects: 0 }
    })
    repositories.jobs.save({
      workId: 'failed-marker', stage: 'transcribed', status: 'failed', attemptCount: 1,
      nextAttemptAt: null, errorCode: 'AI_FAILED', errorMessage: null, updatedAt: '2026-07-12T00:00:00.000Z'
    })
    repositories.artifacts.save({
      workId: 'failed-marker', wavPath: 'managed/failed-marker/audio.wav', transcript: 'saved',
      existingWorkId: null, updatedAt: '2026-07-12T00:00:00.000Z'
    })
    repositories.analyses.save({
      workId: 'failed-marker', transcript: 'saved', result: {}, provider: 'test', model: 'test',
      promptVersion: 'v1', tokenUsage: null, createdAt: '2026-07-12T00:00:00.000Z'
    })
    repositories.snapshots.create({
      id: 'failed-marker-snapshot', workId: 'failed-marker', capturedAt: '2026-07-12T00:00:00.000Z',
      metrics: { likes: 0, comments: 0, shares: 0, collects: 0 }
    })
    const removeManagedWorkDirectory = vi.fn(async () => undefined)
    const service = new ImportService(dependencies({
      removeManagedWorkDirectory,
      onLocalDataChanged: () => { throw new Error('SYNC_STATE_WRITE_FAILED') }
    }))

    await expect(service.deleteFailed('failed-marker')).rejects.toThrow('SYNC_STATE_WRITE_FAILED')

    expect(removeManagedWorkDirectory).toHaveBeenCalledOnce()
    expect(repositories.works.get('failed-marker')).not.toBeNull()
    expect(repositories.jobs.get('failed-marker')).not.toBeNull()
    expect(repositories.artifacts.get('failed-marker')).not.toBeNull()
    expect(repositories.analyses.get('failed-marker')).not.toBeNull()
    expect(repositories.snapshots.listByWork('failed-marker')).toHaveLength(1)
  })
  ...
  it.each([
    ...
    const onLocalDataChanged = vi.fn()
    const service = new ImportService(dependencies({ onLocalDataChanged }))
  ...
  it.each([1, 2])('runs one settled callback after concurrent imports settle when import %i finishes last', async (blockedImport) => {
    ...
    const service = new ImportService(dependencies({
      ...
      processor: {
        extractAudio: vi.fn(async (_id, mediaPath) => {
          if (mediaPath.includes(`managed/${blockedImport}/`)) await secondBlocked
    ...
    const unblocked = blockedImport === 1 ? second : first
    const blocked = blockedImport === 1 ? first : second
    await vi.waitFor(() => expect(repositories.jobs.get(unblocked.workId)?.status).toBe('completed'))
    expect(repositories.jobs.get(blocked.workId)?.status).toBe('running')
