...
describe('Douyin video URL import', () => {
  ...
  it('resolves the iesdouyin share-video redirect returned by real short links', async () => {
    const resolver = vi.fn<ShortLinkResolver>().mockResolvedValueOnce(
      new Response(null, {
        status: 302,
        headers: {
          location: 'https://www.iesdouyin.com/share/video/7628158577101770024/?region=CN&from=web_code_link'
        }
      })
    )
    ...
    await expect(resolveDouyinVideo(
      'https://v.douyin.com/XwkLWciyAgY/',
      { captureSingleVideo: capture },
      resolver,
      noPublicResult
    )).resolves.toMatchObject({
      sourceKey: 'douyin:7628158577101770024',
      originalUrl: 'https://www.douyin.com/video/7628158577101770024'
    })
    expect(capture).toHaveBeenCalledWith(
      '7628158577101770024',
      'https://www.douyin.com/video/7628158577101770024'
    )
  })
