...
import { ModelManager, verifyModelFile } from '../../src/services/asr/model-manager'
...
describe('SenseVoice model manager', () => {
  ...
  it('verifies an existing model file with its size and streaming SHA-256', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'radar-model-'))
    directories.push(directory)
    ...
    await expect(verifyModelFile(manifest, destination)).resolves.toBe(true)
    await expect(verifyModelFile({ ...manifest, sha256: '0'.repeat(64) }, destination)).resolves.toBe(false)
    await expect(verifyModelFile(manifest, join(directory, 'missing.txt'))).resolves.toBe(false)

    const modelDirectory = join(directory, 'directory-instead-of-file')
    mkdirSync(modelDirectory)
    await expect(verifyModelFile({
      ...manifest,
      size: statSync(modelDirectory).size
    }, modelDirectory)).resolves.toBe(false)
  })
