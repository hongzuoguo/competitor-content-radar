...
describe('media downloader URL safety', () => {
  it('accepts an official Douyin static audio CDN URL', async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response('audio-bytes', { status: 200 }))
    const path = await destination()

    await downloadMedia(
      'https://sf11-cdn-tos.douyinstatic.com/obj/ies-music/7578555129511545636.mp3',
      path,
      fetcher
    )

    expect(await readFile(path, 'utf8')).toBe('audio-bytes')
  })

  it('sends browser headers required by Douyin media hosts', async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response('video', { status: 200 }))

    await downloadMedia('https://v26-web.douyinvod.com/video.mp4', await destination(), fetcher)
  ...
  it('accepts a completed local file when the server rejects a range at its exact end', async () => {
    const path = await destination()
    await writeFile(path, '12345')
  ...
  it('restarts a stale partial download when its range is no longer valid', async () => {
    const path = await destination()
    ...
    const fetcher = vi.fn<typeof fetch>()
      ...
      .mockResolvedValueOnce(new Response('fresh-video', { status: 200 }))

    await downloadMedia('https://v3-web.douyinvod.com/video.mp4', path, fetcher)

    expect(fetcher).toHaveBeenCalledTimes(2)
    expect(fetcher.mock.calls[1][1]?.headers).not.toMatchObject({ Range: expect.any(String) })
    expect(await readFile(path, 'utf8')).toBe('fresh-video')
  })
