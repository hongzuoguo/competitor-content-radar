...
export async function downloadMedia(
  ...
  if (response.status === 416 && offset > 0) {
    const total = unsatisfiedRangeTotal(response.headers.get('content-range'))
    await response.body?.cancel().catch(() => undefined)
    if (total === offset) return
    await rm(destination, { force: true })
    return downloadMedia(url, destination, fetchImplementation)
  }
...
function unsatisfiedRangeTotal(value: string | null): number | null {
