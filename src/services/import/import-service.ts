...
import { hasAnalysisSyncChange, hasWorkSyncChange } from '../feishu/local-change'
...
export interface WorkProcessor {
  ...
  analyzeSelected?(workId: string, transcript: string, settings: unknown): Promise<AnalysisOutput>
}
...
export class ImportService {
  ...
  async start(request: ImportRequest): Promise<ImportStartResult> {
    ...
    const work: Work = {
      ...
      ownership: request.ownership ?? 'mine',
    ...
    this.dependencies.repositories.transaction(() => {
      const previous = this.dependencies.repositories.works.get(workId) ?? undefined
      this.dependencies.repositories.works.upsert(work)
      if (hasWorkSyncChange(previous, work)) this.dependencies.onLocalDataChanged?.()
  ...
  private async performFailedDeletion(workId: string): Promise<void> {
    ...
    this.dependencies.repositories.transaction(() => {
      ...
      this.dependencies.onLocalDataChanged?.()
    })
  ...
  private launch(workId: string, request?: ImportRequest): void {
    ...
    const terminal = operation.catch(async (error: unknown) => {
      ...
      const hadActiveImports = this.active.size > 0
      this.active.delete(workId)
      if (hadActiveImports && this.active.size === 0) {
        try {
    ...
    if (work.sourceKey.startsWith('pending:')) {
      ...
      try {
        ...
        if (request.source.type === 'douyin_url' && error instanceof ImportError && error.partialSource) {
          ...
          const updated = this.dependencies.repositories.works.get(workId)
          if (updated && hasWorkSyncChange(previous, updated)) this.dependencies.onLocalDataChanged?.()
          this.emit(workId)
        }
        throw error
      }
  ...
  private async process(workId: string): Promise<void> {
    ...
    if (job.stage === 'transcribed') {
      ...
      const analyze = this.dependencies.processor.analyzeSelected ?? this.dependencies.processor.analyze
      const output = await this.analysisGate.run(() => analyze(workId, transcript, settings))
      repositories.transaction(() => {
        const previous = repositories.analyses.get(workId) ?? undefined
        const analysis = { workId, transcript, ...output, createdAt: now() }
        repositories.analyses.save(analysis)
        if (hasAnalysisSyncChange(previous, analysis)) this.dependencies.onLocalDataChanged?.()
