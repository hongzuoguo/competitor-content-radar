...
const SHARE_REDIRECT_HOSTS = new Set(['iesdouyin.com', 'www.iesdouyin.com'])
...
async function resolveInputUrl(input: string, resolver: ShortLinkResolver): Promise<NormalizedDouyinVideoUrl> {
  ...
  for (let redirect = 0; redirect < MAX_SHORT_LINK_REDIRECTS; redirect += 1) {
    ...
    try {
      ...
      const shareRedirect = normalizeShareRedirect(next)
...
function normalizeShareRedirect(url: URL): NormalizedDouyinVideoUrl | null {
  if (
    url.protocol !== 'https:' ||
    !SHARE_REDIRECT_HOSTS.has(url.hostname) ||
    url.username ||
    url.password ||
    url.port
  ) return null
  const match = /^\/share\/video\/(\d+)\/?$/.exec(url.pathname)
  if (!match) return null
  return {
    videoId: match[1],
    canonicalUrl: `https://www.douyin.com/video/${match[1]}`
  }
}
