...
export function selectAnalysisCandidates(
  works: readonly Work[],
  analyzedWorkIds: ReadonlySet<string>,
  options: AnalysisCandidateOptions
): Work[] {
  const now = options.now ?? new Date()
  const recentWindowMs = options.recentDays * 24 * 60 * 60 * 1000
  const byWorkId = new Map<string, Work>()
  for (const work of works) {
    const existing = byWorkId.get(work.id)
    if (!existing || Date.parse(work.publishedAt) > Date.parse(existing.publishedAt)) {
      byWorkId.set(work.id, work)
    }
  }
  return newestFirst([...byWorkId.values()])
    .filter((work) => !analyzedWorkIds.has(work.id))
    .filter((work) => {
      const age = now.getTime() - Date.parse(work.publishedAt)
      return age >= 0 && age <= recentWindowMs
    })
