...
export class SecretStore {
  constructor(private readonly settings: Pick<SettingsRepository, 'get' | 'set' | 'delete'>) {}
  ...
  has(key: string): boolean {
    return this.settings.get<unknown>(`secret.${key}`) !== null
  }

  delete(key: string): void {
    this.settings.delete(`secret.${key}`)
  }
}
