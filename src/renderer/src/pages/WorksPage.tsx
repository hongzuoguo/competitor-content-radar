...
import type { CreatorView, ImportStartResult, RunFailure, RunHistoryItem, WorkFocusRequest, WorkListItem } from '../../../shared/ipc-contract'
...
import { SubscriptionWorkList, type SubscriptionFilter } from '../features/works/SubscriptionWorkList'
import { WorkInspector } from '../features/works/WorkInspector'
import { RunFailureInspector } from '../features/runs/RunFailureInspector'
import { runFailureDisplayMessage } from '../features/runs/runFailureMessage'
import './workspace-pages.css'

type LoadState = 'loading' | 'ready' | 'failed'
type OwnershipFilter = 'all' | 'mine' | 'competitor'

export function WorksPage({ onImportAccepted, focusRequest, initialFilter = 'all' }: {
  ...
  const [selectedFailure, setSelectedFailure] = useState<{ runId: string; failures: RunFailure[] } | null>(null)
  const failureTriggerRef = useRef<HTMLButtonElement | null>(null)
  const [ownershipFilter, setOwnershipFilter] = useState<OwnershipFilter>('all')
  const [pendingSourceDelete, setPendingSourceDelete] = useState<CreatorView | 'unclassified' | null>(null)
  const importButtonRef = useRef<HTMLButtonElement>(null)
  const deleteDialogRef = useRef<HTMLDialogElement>(null)
  const sourceDeleteDialogRef = useRef<HTMLDialogElement>(null)
  ...
  const loadLatestRun = useCallback(async (): Promise<void> => {
    if (typeof window.desktopApi?.listRuns !== 'function') return
    const runs = await window.desktopApi.listRuns()
    if (mountedRef.current) setLatestRun(runs[0] ?? null)
  }, [])
  ...
  useEffect(() => {
    ...
    void loadCreators()
    void loadLatestRun()
    const handleRunStarted = (): void => { void loadLatestRun() }
    ...
    return () => {
      ...
      cancelFocusRestore()
      unsubscribe()
      window.removeEventListener('content-radar:run-started', handleRunStarted)
    }
  }, [cancelFocusRestore, loadLatestRun, refreshWorks])

  useEffect(() => {
    if (latestRun?.status !== 'running') return
    const timer = window.setInterval(() => { void loadLatestRun() }, 2_000)
    return () => window.clearInterval(timer)
  }, [latestRun?.status, loadLatestRun])
  ...
  useEffect(() => {
    const dialog = sourceDeleteDialogRef.current
    if (!pendingSourceDelete || !dialog || dialog.open) return
    if (typeof dialog.showModal === 'function') dialog.showModal()
    else dialog.setAttribute('open', '')
  }, [pendingSourceDelete])
  ...
  const visibleWorks = useMemo(
    () => ownershipFilter === 'all' ? nonDuplicateWorks : nonDuplicateWorks.filter((work) => work.ownership === ownershipFilter),
    [nonDuplicateWorks, ownershipFilter]
  )
  const visibleCreators = useMemo(
    () => ownershipFilter === 'all'
      ? creators
      : creators.filter((creator) => (creator.ownership ?? 'competitor') === ownershipFilter),
    [creators, ownershipFilter]
  )
  const availableCreatorIds = useMemo(() => {
    const ids = new Set(visibleCreators.filter((creator) => creator.enabled).map((creator) => creator.id))
    for (const work of visibleWorks) {
      ids.add(work.creatorId ?? UNCLASSIFIED_CREATOR_ID)
    }
    return ids
  }, [visibleCreators, visibleWorks])
  const effectiveSelectedCreatorId = selectedCreatorId && availableCreatorIds.has(selectedCreatorId)
    ...
    : ownershipFilter === 'all'
      ? creators.find((creator) => creator.enabled)?.id
        ?? visibleWorks.find((work) => work.creatorId)?.creatorId
        ?? (visibleWorks.some((work) => work.creatorId === null) ? UNCLASSIFIED_CREATOR_ID : null)
      : visibleWorks.find((work) => work.creatorId)?.creatorId
        ?? (visibleWorks.some((work) => work.creatorId === null) ? UNCLASSIFIED_CREATOR_ID : null)
        ?? visibleCreators.find((creator) => creator.enabled)?.id
        ?? null
  const creatorWorks = useMemo(() => {
    const scoped = effectiveSelectedCreatorId === UNCLASSIFIED_CREATOR_ID
      ? visibleWorks.filter((work) => work.creatorId === null)
      : visibleWorks.filter((work) => work.creatorId === effectiveSelectedCreatorId)
    return [...scoped].sort((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt))
  }, [effectiveSelectedCreatorId, visibleWorks])
  ...
  function selectOwnershipFilter(next: OwnershipFilter): void {
  ...
  async function toggleSource(creator: CreatorView): Promise<void> {
    await window.desktopApi.toggleCreator(creator.id, !creator.enabled)
    await loadCreators()
  }

  async function deleteSource(): Promise<void> {
    if (!pendingSourceDelete) return
    if (pendingSourceDelete === 'unclassified') await window.desktopApi.clearUnclassifiedWorks()
    else await window.desktopApi.deleteCreator(pendingSourceDelete.id)
    setPendingSourceDelete(null)
    setSelectedCreatorId(null)
    await Promise.all([loadCreators(), refreshWorks()])
    setMessage('作品来源已删除。')
  }
  ...
  async function retrySelected(runId: string, creatorIds: string[]): Promise<{ accepted: boolean; reason?: string }> {
    ...
    if (result.accepted) {
      setMessage('所选博主的采集任务已启动。')
      void loadLatestRun().catch(() => setRefreshWarning('运行记录刷新失败，任务仍已正常启动。'))
    }
    return result
  }
  ...
  async function retryFeishu(runId: string): Promise<{ accepted: boolean; reason?: string }> {
    ...
    if (result.accepted) {
      setMessage('飞书同步已重新启动。')
      void loadLatestRun().catch(() => setRefreshWarning('运行记录刷新失败，同步仍已正常启动。'))
    }
    return result
  }
  ...
  function closeFailureInspector(): void {
    ...
    requestAnimationFrame(() => {
      if (failureTriggerRef.current?.isConnected) failureTriggerRef.current.focus()
      else worksAreaRef.current?.focus()
    })
  }

  const latestFailure = latestRun?.failures[0] ?? null
  const latestRunTime = latestRun
    ? new Date(latestRun.startedAt).toLocaleString('zh-CN', { hour12: false })
    : ''
  const latestRunResult = latestRun?.status === 'partial'
    ? `${latestRunTime} 部分完成：成功 ${latestRun.analyzed} 条 / 失败 ${latestRun.failures.length} 条`
    : `${latestRunTime} 运行失败`
  ...
  return (
    <div className="page workspace-page">
      ...
      {latestRun?.status === 'running' ? <p aria-live="polite" className="page-message">本次任务正在运行 · 开始于 {latestRunTime}</p> : null}
      {latestFailure && latestRun ? <button className="workspace-failure-banner" onClick={(event) => { failureTriggerRef.current = event.currentTarget; setSelectedFailure({ runId: latestRun.id, failures: latestRun.failures }) }} type="button"><span><strong>{latestRunResult}</strong>共 {latestRun.failures.length} 个失败项 · {latestFailure.creatorName}：{runFailureDisplayMessage(latestFailure)}</span><b>查看失败详情 →</b></button> : null}
      {loadState === 'ready' && nonDuplicateWorks.length > 0 ? <div aria-label="作品归属筛选" className="works-ownership-filter" role="group">
        <span>查看</span>
        <button aria-pressed={ownershipFilter === 'all'} onClick={() => selectOwnershipFilter('all')} type="button">全部作品</button>
        <button aria-pressed={ownershipFilter === 'mine'} onClick={() => selectOwnershipFilter('mine')} type="button">我的作品</button>
        <button aria-pressed={ownershipFilter === 'competitor'} onClick={() => selectOwnershipFilter('competitor')} type="button">对标作品</button>
      </div> : null}
      <div aria-label="作品表格区域" className="works-focus-region" ref={worksAreaRef} role="region" tabIndex={-1}>
        ...
        {loadState === 'ready' && nonDuplicateWorks.length === 0 && creators.length === 0 ? <div className="works-state"><strong>还没有作品</strong><span>导入本地视频或单条抖音作品，完成后会在这里显示拆解结果。</span><Button onClick={() => openImport()}>导入第一个作品</Button></div> : null}
        {loadState === 'ready' && (nonDuplicateWorks.length > 0 || creators.length > 0) ? <div className="subscription-workspace">
          <CreatorRail creators={visibleCreators} onClearUnclassified={() => setPendingSourceDelete('unclassified')} onDeleteRequest={setPendingSourceDelete} onSelect={(id) => { setSelectedCreatorId(id); setSelectedWorkId(null) }} onToggle={(creator) => void toggleSource(creator)} selectedId={effectiveSelectedCreatorId} works={visibleWorks} />
          <SubscriptionWorkList focusId={focusedWorkId} initialFilter={initialFilter} onDeleteRequest={requestDelete} onFocusConsumed={(workId) => setFocusedWorkId((current) => current === workId ? undefined : current)} onLocalFallback={(item) => void chooseLocalFallback(item)} onRetry={retryImport} onSelect={setSelectedWorkId} selectedId={effectiveSelectedWorkId} works={creatorWorks} />
      ...
      {pendingSourceDelete ? <dialog aria-label={pendingSourceDelete === 'unclassified' ? '清空未分类作品？' : `永久删除${pendingSourceDelete.name}？`} className="confirm-dialog" onCancel={(event) => { event.preventDefault(); setPendingSourceDelete(null) }} ref={sourceDeleteDialogRef}>
        <h2>{pendingSourceDelete === 'unclassified' ? '清空未分类作品？' : `永久删除${pendingSourceDelete.name}？`}</h2>
        <p>{pendingSourceDelete === 'unclassified' ? `将删除 ${nonDuplicateWorks.filter((work) => work.creatorId === null).length} 条手动导入作品、文字稿、分析和应用管理的媒体文件。` : '将永久删除该来源的作品、文字稿、分析、指标和应用管理的媒体文件。'}</p>
        <div className="confirm-dialog__actions"><Button onClick={() => setPendingSourceDelete(null)} variant="secondary">取消</Button><Button onClick={() => void deleteSource()} variant="danger">确认永久删除</Button></div>
      </dialog> : null}
      {selectedFailure ? <RunFailureInspector failures={selectedFailure.failures} onClose={closeFailureInspector} runId={selectedFailure.runId} onRetryFeishu={() => retryFeishu(selectedFailure.runId)} onRetrySelected={(creatorIds) => retrySelected(selectedFailure.runId, creatorIds)} /> : null}
    </div>
  )
}
