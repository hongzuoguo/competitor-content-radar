...
export function CreatorsPage({ initialCreators }: { initialCreators?: CreatorRow[] }): React.JSX.Element {
  ...
  async function addMyAccount(profileUrl: string): Promise<void> {
    if (!window.desktopApi?.addMyAccount) {
      setMessage('当前版本暂不支持采集我的作品，请更新应用。')
      return
    }
    setMessage('正在添加我的账号…')
    try {
      const creator = await window.desktopApi.addMyAccount(profileUrl)
      setCreators((current) => current.some((item) => item.id === creator.id)
        ? current.map((item) => item.id === creator.id ? creator : item)
        : [...current, creator])
      setMessage('我的账号已添加，作品会标记为“我的作品”，正在后台进行首次采集。')
    } catch (error) {
      setMessage(error instanceof Error ? error.message : '添加失败，请检查自己的主页地址。')
    }
  }
  ...
  return (
    <div className="page workspace-page">
      ...
      <section aria-label="博主管理工具" className="creator-toolbar">
        <CreatorForm disabled={atLimit} onAdd={(profileUrl) => void addCreator(profileUrl)} onAddMine={(profileUrl) => void addMyAccount(profileUrl)} />
        <p aria-live="polite" className="creator-toolbar__message form-help">{message}</p>
        {atLimit ? <p className="limit-note"><strong>已达到 10 位上限</strong><span>；暂停或删除现有博主后才能继续添加。</span></p> : null}
      </section>
      <section aria-labelledby="creator-monitoring-title" className="creator-monitoring-surface page-section"><div className="section-heading"><div><h2 id="creator-monitoring-title">正在监控</h2><p>关闭监控不会删除历史分析数据</p></div><span>{creators.filter((creator) => creator.enabled).length} 位启用</span></div><CreatorTable creators={creators} onDelete={setPendingDelete} onToggle={(id) => void toggleCreator(id)} /></section>
      {pendingDelete ? (
