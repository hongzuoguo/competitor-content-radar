...
export function ImportWorkDialog({
  ...
  const [ownership, setOwnership] = useState<'mine' | 'competitor'>('mine')
  ...
  async function submit(): Promise<void> {
    ...
    const extractedUrl = sourceType === 'douyin_url' ? extractDouyinWorkUrl(url) : null
    const validation = validateSource(sourceType, localPath, url, extractedUrl)
    if (validation) {
    ...
    try {
      const result = await window.desktopApi.startImport({
        source: sourceType === 'local'
          ...
          : { type: 'douyin_url', url: extractedUrl! },
  ...
  return (
    ...
    >
      ...
      <div aria-label="内容来源" className="import-source-tabs" role="group">
        ...
        <button aria-pressed={sourceType === 'douyin_url'} onClick={() => selectSource('douyin_url')} onKeyDown={(event) => switchTabWithArrow(event, 'local', localTabRef, selectSource)} ref={urlTabRef} type="button"><Link2 size={16} aria-hidden="true" />抖音分享</button>
      </div>

      <div className="import-dialog__body">
        <fieldset className="ownership-field">
          <legend>作品归属</legend>
          <div>
            <label><input checked={ownership === 'mine'} name="ownership" onChange={() => setOwnership('mine')} type="radio" />我的作品</label>
            <label><input checked={ownership === 'competitor'} name="ownership" onChange={() => setOwnership('competitor')} type="radio" />对标作品</label>
          </div>
          <small>用于在本地和飞书中分开查看，不会影响分析方式。</small>
        </fieldset>
        ...
        {sourceType === 'local' ? (
          ...
          <div className="form-field">
            <label htmlFor="import-douyin-url">抖音分享内容或作品链接</label>
            <textarea
              ...
              placeholder="粘贴完整分享内容或 https://www.douyin.com/video/..."
              value={url}
            />
            {fieldError ? <span className="form-error" id="import-url-error" role="alert">{fieldError}</span> : <span className="form-help" id="import-url-help">可粘贴完整分享口令或单条公开作品链接，不支持纯博主主页。</span>}
          </div>
        )}
...
function validateSource(sourceType: SourceType, localPath: string, rawUrl: string, extractedUrl: string | null): string {
  ...
  try {
    ...
    if (
      ...
      !parsed.username &&
      !parsed.password &&
      (parsed.hostname === 'douyin.com' || parsed.hostname === 'www.douyin.com') &&
      /^\/user\/[^/]+\/?$/.test(parsed.pathname) &&
      !parsed.searchParams.has('modal_id') &&
      !parsed.searchParams.has('vid')
    ) {
      return '请输入抖音作品分享内容，不支持纯博主主页。'
    }
  } catch {}
  return '没有在分享内容中识别到抖音作品链接。'
}
